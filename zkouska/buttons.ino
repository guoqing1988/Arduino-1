...
long last_time_pressed[BUTTON_COUNT] = {0, 0, 0};
...
bool is_held(int button) {
	return (is_pressed(button) && (millis() - last_time_pressed[button]) > HELD_LENGTH);
}
...
void update_button_buffer_state() {
	...
	for (int i = 0; i < BUTTON_COUNT; i++) {
		...
		// record last times pressed
		if (was_pressed(i))
			last_time_pressed[i] = millis();
	}
}
